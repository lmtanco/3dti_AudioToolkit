// testHL-06e: forensic comparison of two band-limit configurations of the
// Gammatone hearing loss simulator, using the same audiometry (single-band impulse audiometries):
//
...
//    -> testHL-06e-bandConfig.mat
//  - the stereo outputs for inputs scaled x0.5, x1, x2, x4, x8
//    -> testHL-06e-stereoOutputs.mat
...
int main()
{
    ...
    // Single-band "impulse" audiometries, to expose each configuration bug in isolation:
    //  - Left: 80 dBHL at index 3 (clinical 2000 Hz). Expected in "exp": the attenuation
    //    lands on octave band 1000 (issue #17), the 1000 Hz group loses it entirely
    //    (GetBandGainDB factor==0 fallthrough) while keeping a huge spurious ratio, and
    //    the 2000 Hz group only recovers a fraction via the drifted center (1861 Hz).
    //  - Right: 80 dBHL at index 6 (clinical 6000 Hz). Expected in "exp": shift+drift
    //    without the fallthrough: attenuation lands on octave 8000, recovered only
    //    partially by the 6000 Hz group.
    std::vector<float> audiometryLeft  = {0, 0, 0, 80, 0, 0, 0, 0};
    std::vector<float> audiometryRight = {0, 0, 0, 0, 0, 0, 80, 0};
    ...
    {
        matioCpp::File file = matioCpp::File::Create("testHL-06e-bandConfig.mat");
    ...
    {
        matioCpp::File file = matioCpp::File::Create("testHL-06e-stereoOutputs.mat");
