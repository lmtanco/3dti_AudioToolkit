function plot_output_testHL_06d()
...
% Severe audiometry: L = {40,35,45,75,65,80,80,85}, R = {30,25,50,75,75,85,80,85}.

fs = 48000;
cfgData = open('../build/Debug/testHL-06d-bandConfig.mat');
outData = open('../build/Debug/testHL-06d-stereoOutputs.mat');

% Print the configured expander parameters for both configurations
printConfig(cfgData, 'oct', 'SEVERE — OCTAVE-ALIGNED band limits (no index mismatch)');
printConfig(cfgData, 'exp', 'SEVERE — EXPERIMENT band limits (band-index mismatch)');
...
figure('Name', 'SEVERE — TF comparison: octave-aligned vs experiment band limits');
plotIdx = 0;
for e = 1:2
    for c = 1:2
        plotIdx = plotIdx + 1;
