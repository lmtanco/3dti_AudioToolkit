function plot_output_testHL_06c()
...
% Moderate audiometry: L = {15,5,15,40,55,65,70,55}, R = {15,5,15,45,55,60,55,85}.

fs = 48000;
cfgData = open('../build/Debug/testHL-06c-bandConfig.mat');
outData = open('../build/Debug/testHL-06c-stereoOutputs.mat');

% Print the configured expander parameters for both configurations
printConfig(cfgData, 'oct', 'MODERATE — OCTAVE-ALIGNED band limits (no index mismatch)');
printConfig(cfgData, 'exp', 'MODERATE — EXPERIMENT band limits (band-index mismatch)');
...
figure('Name', 'MODERATE — TF comparison: octave-aligned vs experiment band limits');
plotIdx = 0;
for e = 1:2
    for c = 1:2
        plotIdx = plotIdx + 1;
