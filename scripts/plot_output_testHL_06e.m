function plot_output_testHL_06e()
...
% impulse audiometries: L = 80 dBHL at index 3 (clinical 2000 Hz), R = 80 dBHL at index 6 (clinical 6000 Hz).

fs = 48000;
cfgData = open('../build/Debug/testHL-06e-bandConfig.mat');
outData = open('../build/Debug/testHL-06e-stereoOutputs.mat');

% Print the configured expander parameters for both configurations
printConfig(cfgData, 'oct', 'IMPULSE — OCTAVE-ALIGNED band limits (no index mismatch)');
printConfig(cfgData, 'exp', 'IMPULSE — EXPERIMENT band limits (band-index mismatch)');
...
figure('Name', 'IMPULSE — TF comparison: octave-aligned vs experiment band limits');
plotIdx = 0;
for e = 1:2
    for c = 1:2
        plotIdx = plotIdx + 1;
